...
- 任意调用方不得直接拼装或覆盖 payload。
- 若出现绕过 _build_payload(...) 的实现，判定为发布阻断缺陷。

## 18. Async Round-Exit Rule (Release Blocking)
- On submit success (status==200 and non-empty jobId), current agent round must terminate immediately.
- Same-round polling/waiting after submit success is forbidden.
- System should expose machine-readable flags (ound_complete=true, 
otify_async=true) to drive deterministic agent behavior.
