...
### Fixed
- Cleaned core/api.py atmosphere payload branch to remove manually mixed fields and keep workflow request body fully generated by _build_payload(...).
- Replaced invalid 
ull literal with Python None in payload construction path.

### Changed
- Added hard guard in submit_task(...): direct payload override is rejected so callers cannot bypass _build_payload(...).
- Added explicit payload-construction constraint to SKILL.md and SPEC.md as a release-blocking rule.
